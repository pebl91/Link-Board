class MicropostsController < ApplicationController
  ...
  def create
    ...
    add_tags(@micropost, @micropost.tag_list)
    if @micropost.save
  ...
  def add_tags(micropost, tag_string)
    return if tag_string.blank?
    tag_string.split.each do |tag_name|
      normalized = tag_name.delete_prefix("#").downcase
      tag = Tag.find_or_create_by!(name: normalized)
      micropost.tags << tag unless micropost.tags.include?(tag)
    end
  end
end
